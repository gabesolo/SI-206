...
def get_umsi_data():
    ...
    html = Request(url, headers={'User-Agent': 'SI_CLASS'})
    page = urlopen(html).read()
    soup = BeautifulSoup(page, 'html.parser')
    content = soup.find_all(class_="ds-1col node node-person node-teaser view-mode-teaser clearfix")
    name_list = []
    title_list = []
    for x in content:
        name = x.find(class_='field field-name-title field-type-ds field-label-hidden')
    ...
    i = 1
    while i <= 12:
        for a in next_url.find_all('a', href=True):
            new_url = "https://www.si.umich.edu/" + (a['href'])
        html = Request(new_url, headers={'User-Agent': 'SI_CLASS'})
        page = urlopen(html).read()
        soup = BeautifulSoup(page, 'html.parser')
        content = soup.find_all(class_="ds-1col node node-person node-teaser view-mode-teaser clearfix")
        name_list = []
